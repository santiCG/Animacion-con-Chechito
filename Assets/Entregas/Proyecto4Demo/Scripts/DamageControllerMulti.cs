...
public class DamageControllerMulti : MonoBehaviour
{
    private List<DamageMessage> damageList = new List<DamageMessage>();

    private bool ignoreDamage;
    private Animator animator;
    private ChracterStateALR playerState;

    private void Awake()
    {
        animator = GetComponent<Animator>();
        playerState = GetComponent<ChracterStateALR>();
    }
    ...
    private void Update()
    {
        ...
        {
            playerState.DepleteHealth(message.amount, out bool dead); // esta mal, pq solo estamos accediendo al player1,
